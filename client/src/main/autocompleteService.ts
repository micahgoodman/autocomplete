...
export class AutocompleteService {
  ...
  async completeTask(taskText: string): Promise<AutocompleteResponse> {
    ...
    try {
      ...
      for await (const message of stream) {
        ...
        if (message.type === 'assistant') {
          ...
          if (this.isLikelyDraft(cleaned)) {
            ...
            hasLockedDraft = true;
            console.log('[AutocompleteService] Locked draft from assistant message');
          } else {
            console.log('[AutocompleteService] Assistant text rejected by heuristics');
          }
        }
      }
      ...
      if (!hasLockedDraft) {
        console.warn('[AutocompleteService] No clean draft extracted; returning failure');
        return {
          completedText: taskText,
          success: false,
  ...
  private extractDraftBody(text: string): string | null {
    ...
    if (codeBlockMatch && !/^(bash|sh|shell)$/i.test(codeBlockMatch[0].match(/```([^\n]*)/)?.[1] || '')) {
      return codeBlockMatch[1].trim();
    }

    // Look for structured content (markdown headings, lists, etc.)
    // This is likely the actual draft, not meta-commentary
    const hasStructure = /^#{1,3}\s+/m.test(text) || /^\d+\.\s+/m.test(text) || /^[-*]\s+/m.test(text);
    ...
    if (hasStructure) {
      ...
      const metaLineRe = /^(Let me|I\'?ve|I have|Would you like|This (draft|guide)|The draft)/i;
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (firstContentLine === -1 && (line.startsWith('#') || /^\d+\./.test(line) || /^[-*]/.test(line) || line.length > 40)) {
          firstContentLine = i;
        }
    ...
    let cleaned = text.replace(/^(Here\'?s a draft[^:]*:|Draft[^:]*:|Here is[^:]*:|Let me write[^:]*:)\s*/i, '');

    // Drop obvious meta sentences at the start and end
    cleaned = cleaned.replace(/^(I\'?ve|I have|Would you like me|Let me (know|mark|write))[^\n]*\n?/gi, '').trim();
    ...
    const metaRe = /(I\'?ve|I have|Would you like me|this draft|the draft is|I can revise|let me (know|mark))/i;
  ...
  private isLikelyDraft(text: string): boolean {
    ...
    const commentarySignals = /(I\'?ve|I have|Would you like me|this draft|the draft is|I can revise|let me (know|mark))/i.test(text);
